...
<body class="bg-gray-100">
    <div class="flex min-h-screen">
        <!-- Sidebar -->
        <aside
            class="w-64 bg-gradient-to-b from-slate-800 to-slate-900 text-white fixed inset-y-0 left-0 z-50 transform transition-transform duration-300 ease-in-out lg:translate-x-0"
            id="sidebar">
                ...
                @endauth
                    ...
                    @auth
                        @if(auth()->user()->role == 'penerima')
                            ...
                            <a href="/pendaftaran/create"
                                class="flex items-center px-4 py-3 text-sm font-medium rounded-lg hover:bg-slate-700 transition-colors">
                                <i class="fas fa-table w-5 h-5 mr-3"></i>
                                Daftar PKH
                            </a>
                            ...
                            <!-- Info Pendamping -->
                            <a href="{{ route('pendamping.infoPendamping') }}"
                                class="flex items-center px-4 py-3 text-sm font-medium rounded-lg hover:bg-slate-700 transition-colors">
                    ...
                    @auth
                        @if(auth()->user()->role == 'pendamping')
                            ...
                            <a href="{{ route('dashboard') }}"
                                class="flex items-center px-4 py-3 text-sm font-medium rounded-lg hover:bg-slate-700 transition-colors {{ request()->routeIs('pendamping.index') ? 'bg-slate-700' : '' }}">
                                <i class="fas fa-tachometer-alt w-5 h-5 mr-3"></i>
                                Dashboard Pendamping
                            </a>
                            ...
                            <a href="{{ route('pendamping.laporan') }}"
                                class="flex items-center px-4 py-3 text-sm font-medium rounded-lg hover:bg-slate-700 transition-colors {{ request()->routeIs('pendamping.laporan') ? 'bg-slate-700' : '' }}">
                ...
                <div class="flex items-center justify-between px-6 py-4">
                    <div class="flex items-center">
                        ...
                        <div class="relative hidden md:block">
                            ...
                            <input type="text"
                                class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg bg-gray-50 text-sm focus:outline-none focus:ring-2 focus:ring-slate-500 focus:border-transparent"
                                placeholder="Search...">
                        </div>
                    </div>
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="relative">
                            <button class="p-2 text-gray-600 hover:text-gray-900 relative">
                                ...
                                <span
                                    class="absolute -top-1 -right-1 h-4 w-4 bg-red-500 text-white text-xs rounded-full flex items-center justify-center">7</span>
                            </button>
                        </div>
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open"
                                ...
                                <img class="h-8 w-8 rounded-full mr-2"
                                    src="{{ asset('sbadmin2/img/undraw_profile.svg') }}" alt="Profile">
